...
**Tool Result:**
```json
{"type": "tool", "id": "call_abc123", "tool_name": "add_threats", "tool_start": false, "content": {"response": [{"name": "SQL Injection"}]}, "error": false}
```
...
**Error Response:**
```json
{"type": "error", "content": "This conversation was started with OpenAI but the system is now configured to use Amazon Bedrock. Please start a new conversation."}
```
