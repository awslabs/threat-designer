...
function _createOpenAIModel(modelConfig, reasoning, reasoningModels, apiKey, modelName) {
  try {
    ...
    if (reasoning > 0 && reasoningModels.includes(modelConfig.id)) {
      // Determine which reasoning effort map to use based on model type
      const isMiniModel = modelConfig.id.includes("mini");
      const reasoningEffortMap = isMiniModel
        ? OPENAI_REASONING_EFFORT_MAP_MINI
        : OPENAI_REASONING_EFFORT_MAP_STANDARD;

      const reasoningEffort = reasoningEffortMap[reasoning];
      if (reasoningEffort) {
        config.reasoning_effort = reasoningEffort;
        console.log(
          `Reasoning enabled for ${modelName}: effort=${reasoningEffort}, model=${modelConfig.id} (${isMiniModel ? "mini" : "standard"} mapping)`
        );
      }
    } else if (reasoning > 0) {
      console.warn(
