resource "aws_bedrockagentcore_agent_runtime" "sentry" {
    ...
    role_arn                  = aws_iam_role.sentry_role.arn
    environment_variables     = {
        SESSION_TABLE   = aws_dynamodb_table.sentry_session.id,
    ...
    agent_runtime_artifact {
        container_configuration {
            container_uri = "${aws_ecr_repository.sentry.repository_url}:latest"
        }
    }
...
resource "null_resource" "docker_build_push" {
  ...
  provisioner "local-exec" {
    ...
    command = <<-EOT
      ...
      aws ecr get-login-password --region ${var.region} | docker login --username AWS --password-stdin ${aws_ecr_repository.sentry.repository_url}
      
      # Build image
      docker build --build-arg AWS_REGION=${var.region} -t ${aws_ecr_repository.sentry.name}:latest .
      
      # Tag image
      docker tag ${aws_ecr_repository.sentry.name}:latest ${aws_ecr_repository.sentry.repository_url}:latest
      
      # Push image
      docker push ${aws_ecr_repository.sentry.repository_url}:latest
    EOT
  }
}
  ...
  policy = jsonencode({
    ...
    "Statement" : [
      ...
      },
      {
        ...
        "Resource": [
            "${aws_dynamodb_table.sentry_session.arn}",
            "${aws_dynamodb_table.sentry_session.arn}/*"
        ]
        },
    {
