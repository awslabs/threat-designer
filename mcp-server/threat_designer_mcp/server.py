...
async def app_lifespan(server: FastMCP) -> AsyncIterator[AppContext]:
    ...
    client = httpx.AsyncClient(headers={"x-api-key": api_key}, timeout=30.0)
...
mcp = FastMCP(
    "threat-designer.mcp-server", dependencies=["pydantic"], lifespan=app_lifespan
)
...
async def get_threat_model(
    ...
    model_id: str,
    filter: Optional[
        Literal["threats", "assets", "trust_boundaries", "threat_sources"]
    ],
) -> str:
    """
    ...
    try:
        ...
        if (
            status_data.get("state") == "Found"
            and filter is not None
            and "item" in status_data
        ):
            item = status_data["item"]
            filtered_item = {}

            if filter == "threats" and "threat_list" in item:
                filtered_item["threat_list"] = {
                    "threats": item["threat_list"]["threats"]
                }
        ...
        return json.dumps(
            {"job_id": model_id, "state": "ERROR", "message": error_message}
        )
    ...
    try:
        ...
        if "arch_location" in payload_dict and payload_dict["arch_location"]:
            ...
            presigned_response = await app_context.api_client.post(
                f"{app_context.base_endpoint}/upload", json={"file_type": content_type}
            )
            ...
            async with httpx.AsyncClient() as client:
                upload_response = await client.put(
                    presigned_data["presigned"], content=file_data, headers=headers
                )
        ...
        response = await app_context.api_client.post(
            f"{app_context.base_endpoint}", json=payload_dict
        )
        response.raise_for_status()
...
async def check_threat_model_status(ctx: Context, model_id: str) -> str:
    ...
    try:
        ...
        error_message = f"Error checking status: {str(e)}"
        return json.dumps({"id": model_id, "status": "ERROR", "message": error_message})
