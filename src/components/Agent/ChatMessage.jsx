...
//   // If starting a new tool while previous tool is incomplete,
//   // mark the previous as interrupted/failed
//   if (item.tool_start && lastBlock?.type === "tool" && !lastBlock.isComplete) {
//     lastBlock.isComplete = true;
//     lastBlock.interrupted = true; // Add a flag to indicate interruption
//   }
  ...
  const messageBlocks = useMemo(() => {
    ...
    for (let i = 0; i < message.length; i++) {
      ...
      if (item.type === "tool") {
        ...
        // Check if we should update an existing tool block
        const shouldUpdateExisting =
          lastBlock &&
          lastBlock.type === "tool" &&
          lastBlock.toolName === item.tool_name &&
        ...
        if (shouldUpdateExisting) {
          ...
          if (item.tool_update) {
            lastBlock.input = item.content; // Set input from the update
            lastBlock.items.push(item);
          }
          ...
          // Handle duplicate tool_start - mark previous as interrupted
            ...
            lastBlock.isComplete = true;
            lastBlock.interrupted = true; // Mark as interrupted
