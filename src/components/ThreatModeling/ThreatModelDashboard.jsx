 ...
 * @param {Array} props.threats - Array of threat objects from the threat model
 * @returns {JSX.Element} The rendered dashboard with statistical charts
 */
const ThreatModelDashboard = ({ threats = [] }) => {
  // Validate input data and filter out malformed threat objects
  const validThreats = useMemo(() => {
    if (!Array.isArray(threats)) {
      console.warn("ThreatModelDashboard: threats is not an array", typeof threats);
      return [];
    }
    ...
    if (filtered.length < threats.length) {
      console.warn(
        `ThreatModelDashboard: Filtered out ${threats.length - filtered.length} invalid threat(s)`
      );
    }
 ...
 * Only re-render if the threats prop has actually changed
 */
const arePropsEqual = (prevProps, nextProps) => {
  // Handle null/undefined cases
  if (!prevProps.threats && !nextProps.threats) {
    return true;
  }
  if (!prevProps.threats || !nextProps.threats) {
    return false;
  ...
  if (prevProps.threats.length !== nextProps.threats.length) {
    return false;
  }
  ...
  for (let i = 0; i < prevProps.threats.length; i++) {
    if (prevProps.threats[i] !== nextProps.threats[i]) {
      return false;
    }
  }
