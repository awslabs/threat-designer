...
class TestInvokeLambda:
    ...
    def test_invoke_lambda_creates_agent_state_and_job_status(
        ...
        assert call_args[1]['agentRuntimeArn'] == 'arn:aws:bedrock-agent:us-east-1:123456789012:agent/test-agent'
    ...
    def test_invoke_lambda_creates_backup_before_replay(self, mock_dynamodb, mock_status_table, mock_agent_client, mock_uuid):
        ...
        mock_agent_table.get_item.assert_called_once_with(Key={'job_id': 'existing-job-123'})
    ...
    def test_invoke_lambda_handles_invocation_errors(
        ...
        mock_agent_client.invoke_agent_runtime.side_effect = Exception('Invocation failed')
...
class TestFetchResults:
    ...
    @patch.object(threat_designer_service, 'dynamodb')
    def test_fetch_results_returns_threat_model_for_owner(self, mock_dynamodb, mock_check_access, sample_threat_model):
    ...
    @patch.dict('os.environ', {'AGENT_STATE_TABLE': 'test-agent-table'})
    @patch.object(threat_designer_service, 'dynamodb')
    def test_fetch_results_mcp_user_bypasses_authorization(self, mock_dynamodb, sample_threat_model):
...
class TestUpdateResults:
    ...
    def test_update_results_owner_can_update_with_valid_lock(
        self, mock_datetime, mock_dynamodb, mock_update_item, mock_hash, 
        mock_lock_status, mock_require_access, sample_threat_model
    ):
        ...
        mock_update_item.return_value = {'job_id': 'test-job-123', 'description': 'Updated'}
    ...
    def test_update_results_collaborator_with_edit_access_can_update(
        ...
        # Assert
        mock_require_access.assert_called_once_with('test-job-123', 'user-456', required_level='EDIT')
    ...
    def test_update_results_preserves_timestamp_when_no_content_change(
        self, mock_dynamodb, mock_update_item, mock_hash, mock_lock_status, mock_require_access, sample_threat_model
    ):
...
class TestDeleteThreatModel:
    ...
    def test_delete_tm_owner_can_delete(
        self, mock_dynamodb, mock_delete_s3, mock_delete_db, mock_fetch,
        mock_check_status, mock_lock_status, mock_require_owner
    ):
        ...
        mock_fetch.return_value = {
            'item': {'s3_location': 'test-key.json'}
        }
    ...
    def test_delete_tm_force_releases_lock_if_requested(
        self, mock_dynamodb, mock_delete_s3, mock_delete_db, mock_fetch,
        mock_check_status, mock_force_release, mock_lock_status, mock_require_owner
    ):
    ...
    def test_delete_tm_stops_active_execution_before_deletion(
        self, mock_dynamodb, mock_delete_s3, mock_delete_db, mock_fetch,
        mock_delete_session, mock_check_status, mock_lock_status, mock_require_owner
    ):
    ...
    def test_delete_tm_cleans_up_sharing_records(
        ...
        mock_sharing_table.batch_writer.return_value.__enter__ = Mock(return_value=mock_batch_writer)
